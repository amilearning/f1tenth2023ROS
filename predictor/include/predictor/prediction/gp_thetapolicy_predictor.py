...
# from predictor.prediction.gp_encoder.ThetaGPModel import ThetaGPTrained
from predictor.prediction.gp_encoder.gp_policyEncoder import GPContPolicyEncoder 
from predictor.prediction.gp_encoder.gp_encoderdataGen import states_to_encoder_input_torch

class GPThetaPolicyPredictor(BasePredictor):
    def __init__(self, N: int, track : RadiusArclengthTrack, policy_name: str, use_GPU: bool, M: int, model=None, cov_factor: float = 1,cont_encoder = True):
        super(GPThetaPolicyPredictor, self).__init__(N, track)
        gc.collect()
        ...
        self.policy_encoder = GPContPolicyEncoder(model_load = True, model_id = encoder_model)        
        print("policy extractor loaded")
        ######### Input prediction for Gaussian Processes regression ######### 
        input_predict_model = "GPEncoder"
    ...
    def get_prediction(self, ego_state: VehicleState, target_state: VehicleState,
        ...
        if is_encoder_input_ready: ## encoder_is_ready = True
            pred = self.policy_encoder.get_true_prediction_par(self.encoder_input,  ego_state, target_state, ego_prediction, self.track, self.M)            
