...
with torch.no_grad() and torch.cuda.amp.autocast():
    ...
    ###############################################################################
        ...
        def kinematic_residual_state_update(self,x,v,delta_xv): 
            # x: s(0), ey(1), epsi(2), 
            # v: vx(0), vy(1), wz(2)
            # delta_x: kinamaticupdate_ey  - nstate_ey , kinamaticupdate_epsi  - nstate_epsi, del_vx
            ...
            nx[:,0] = s + self.dt * ( (vx * torch.cos(epsi) - vy * torch.sin(epsi)) / (1 - curs * ey) )
            nx[:,1] = ey + self.dt * (vx * torch.sin(epsi) + vy * torch.cos(epsi)) - delta_ey
            nx[:,2] = epsi + self.dt * ( wz - (vx * torch.cos(epsi) - vy * torch.sin(epsi)) / (1 - curs * ey) * curs ) - delta_epsi
