...
import numpy as np
import torch
from barcgp.prediction.gp_encoder.gp_encoderdataGen import SampleGeneartorGPContEncoder
from barcgp.prediction.gp_encoder.gp_policyEncoder import GPContPolicyEncoder
from torch.utils.data import DataLoader, random_split


# Training
def gp_encoder_train(dirs):

    sampGen = SampleGeneartorGPContEncoder(dirs, randomize=True)
    ...
    policy_encoder.model_training(args = args_)
    
    create_dir(path=model_dir)
    policy_encoder.model_save()
    policy_encoder.model_load()
...
def tsne_cont_encoder(dirs):
    ...
    for i in range(len(dirs)):
        dir = [dirs[i]]
        a_sampGen = SampleGeneartorGPContEncoder(dir, randomize=True)
        ...
        a_policy_encoder = GPContPolicyEncoder(args= args_)    
