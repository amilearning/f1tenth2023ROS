...
from predictor.common.pytypes import VehicleState
...
def drawElipse(ax,mean,std):
    ...
    # std = np.array([3, 1])   # standard deviation
    # create an ellipse patch
    ellipse = Ellipse(xy=mean, width=2*std[0], height=2*std[1], angle=0, fill=False, linewidth=2)
    # add the ellipse patch to the axes object
    ax.add_patch(ellipse)
    # # set the x and y limits of the axes object
    # ax.set_xlim(mean[0]-4*std[0], mean[0]+4*std[0])
    # ax.set_ylim(mean[1]-4*std[1], mean[1]+4*std[1])    
...
def get_curvature_from_keypts_torch(s,track):
    ...
    # return torch.tensor(track.key_pts)[0,5].view(1, -1).to(device="cuda").clone()
