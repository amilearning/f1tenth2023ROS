...
import numpy as np
import torch
from barcgp.common.utils.scenario_utils import SampleGeneartorEncoder, SampleGenerator, Sample
...
def main():
    ...
    aggressive_dirs = [a_scencurve_dir]    
    timid_dirs = [t_scencurve_dir]    
    
    a_sampGen = SampleGeneartorEncoder(dirs, randomize=True)
    t_sampGen = SampleGeneartorEncoder(dirs, randomize=True)
    
    
    if not dir_exists(a_scencurve_dir) or not dir_exists(t_scencurve_dir):
        raise RuntimeError(
            f"Directory: {a_scencurve_dir} or {t_scencurve_dir} does not exist, need to train using `gen_training_data` first")

    train_dataset, val_dataset, test_dataset  = a_sampGen.get_datasets()
    train_dataset, val_dataset, test_dataset  = t_sampGen.get_datasets()
