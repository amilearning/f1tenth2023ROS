...
from predictor.common.pytypes import VehicleState, VehiclePrediction, ParametricPose, BodyLinearVelocity, VehicleActuation
from predictor.common.tracks.radius_arclength_track import RadiusArclengthTrack
from predictor.common.tracks.track_lib import CurveTrack, StraightTrack, ChicaneTrack
from predictor.common.tracks.track import get_track
...
class SampleGeneartorIKDTime(SampleGenerator):
    ...
    def plotStatistics(self):
        ...
        for i in self.output_data:
            # data_list.append(i.input[1].lookahead.curvature[0])
            accel_list.append(i[0])
            steer_list.append(i[1])
        fig, axs = plt.subplots(2, 1)
        axs[0].hist(accel_list, bins=50)
        axs[1].hist(steer_list, bins=50)
        plt.show()
