...
class RacingGP():
    ...
    def NNmodelLoad(self,args = None, model_name = 'lstmvae524067e7.model'):
        ...
        if args is None:
            ...
            args =  {
                        ...
                        "input_size": self.train_dataset[0].shape[1]-2,
                        "hidden_size": self.train_dataset[0].shape[1],
                        "latent_size": 2,
        ...
        if self.LatentPolicy:
            ...
            print("No need to load NN Model as Latenetpolicy is False")
    ...
    def dataloader_preprocess(self,dataloader = None):        
        ...
        for count, data_batch in enumerate(dataloader):              
            ...
            batch_data_nn = data_batch[:,:,0:6].to(torch.device("cuda"))    
    ...
    def setupGPModel(self,model_load = False):
        ...
        self.model_delta = GPModel(inducing_points=self.onerow_x_data[:self.n_inducing,:], Distributional_GP = self.Distributional_GP).to(torch.device("cuda"))           
        self.likelihood_delta = gpytorch.likelihoods.GaussianLikelihood().to(torch.device("cuda"))   
        self.optimizer_delta = torch.optim.Adam(self.model_delta.parameters(), lr=self.lr)  # Includes GaussianLikelihood parameters        
        self.mll_delta = gpytorch.mlls.VariationalELBO(self.likelihood_delta, self.model_delta, num_data=self.y_delta.size(0))
    ...
    def GPtrain(self,n_epoch):                        
        ...
        self.model_delta.train()
        self.likelihood_delta.train()
        ...
        for i in range(training_iter):
            ...
            y_accel_batch = self.batch_y_data[batch_count][:,0]
            loss = -self.mll_accel(output, y_accel_batch)
            loss.backward()
            ...
            if converge_count > self.converge_count_thres: 
                model_accel_file_name = os.path.join(model_dir, 'model_accel_'+str(i) +'.pth')                
                likelihood_accel_file_name = os.path.join(model_dir, 'liklihood_accel_'+str(i) +'.pth')
    ...
    def validation(self):
        ...
        for i in range(self.test_batch_x_data.shape[0]):
            ...
            mse_val_loss = mseloss_for_testing(acc_observed_pred.mean, y_test[:,0].squeeze())
...
datafilename = 'trainx_0_partial.pkl'
