...
class VehicleMPPI():
    ...
    def __init__(self, dynamics, running_cost, nx, noise_sigma, num_samples=100, horizon=15, device="cuda",
        ...
        self.dyn_params = torch.ones(7).to(device=self.d)
    ...
    @handle_batch_input
    def _running_cost(self, state, u, t,prev_state, prev_u):
        return self.running_cost(state, u, t,prev_state, prev_u)
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        for t in range(T):            
            ...
            c = self._running_cost(state, u, t, prev_state, prev_u)
            prev_u = u
    ...
    def _compute_total_cost_batch(self):
        ...
        if self.noise_abs_cost:
            ...
            noise_tensor = self.noise @ self.noise_sigma_inv # Like original paper
            action_cost = noise_tensor
            action_cost[:,:,0] = action_cost[:,:,0]*self.lambda_*self.dyn_params[7]  # 5m/s for 1e-3 and 1 
            action_cost[:,:,1] = action_cost[:,:,1]*self.lambda_*self.dyn_params[8]
