...
% 
classdef gp_mpcc_h2h_egoBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'gp_mpcc_h2h_ego'))
                solverpath = fullfile(buildablepath, 'gp_mpcc_h2h_ego');
            end
            solverInfo = struct();
            solverInfo.solvername = 'gp_mpcc_h2h_ego';
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x01 = coder.nullcopy(zeros(19, 1));
            outputs_x02 = coder.nullcopy(zeros(19, 1));
            outputs_x03 = coder.nullcopy(zeros(19, 1));
            outputs_x04 = coder.nullcopy(zeros(19, 1));
            outputs_x05 = coder.nullcopy(zeros(19, 1));
            outputs_x06 = coder.nullcopy(zeros(19, 1));
            outputs_x07 = coder.nullcopy(zeros(19, 1));
            outputs_x08 = coder.nullcopy(zeros(19, 1));
            outputs_x09 = coder.nullcopy(zeros(19, 1));
            outputs_x10 = coder.nullcopy(zeros(19, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = gp_mpcc_h2h_egoBuildable.forcesCall(params.x0, params.xinit, params.all_parameters);
        end

        function [output,exitflag,info] = forcesCall(x0, xinit, all_parameters)
            solvername = 'gp_mpcc_h2h_ego';
            ...
            [output, exitflag, info] = gp_mpcc_h2h_egoBuildable.forcesInitOutputsMatlab();
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x01 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x02 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x03 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x04 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x05 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x06 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x07 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x08 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x09 = coder.nullcopy(double(zeros(19, 1)));
            outputs_x10 = coder.nullcopy(double(zeros(19, 1)));
            output = struct('x01', outputs_x01,...
                            'x02', outputs_x02,...
                            'x03', outputs_x03,...
                            'x04', outputs_x04,...
