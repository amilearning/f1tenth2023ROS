...
typedef gp_mpcc_h2h_ego_float solver_float;
...
void copyCArrayToM_gp_mpcc_h2h_ego(gp_mpcc_h2h_ego_float *src, double *dest, solver_int32_default dim) 
{
...
void copyCValueToM_gp_mpcc_h2h_ego(gp_mpcc_h2h_ego_float* src, double* dest)
{
    *dest = (double)*src;
}

void copyMArrayToC_gp_mpcc_h2h_ego(double *src, gp_mpcc_h2h_ego_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
...
void copyMValueToC_gp_mpcc_h2h_ego(double * src, gp_mpcc_h2h_ego_float * dest)
{
	*dest = (gp_mpcc_h2h_ego_float) *src;
}
...
{
    const solver_int nvarArr[NSTAGES] = {19, 19, 19, 19, 19, 19, 19, 19, 19, 19}; 
    const solver_int neqArr[NSTAGES] = {14, 14, 14, 14, 14, 14, 14, 14, 14, 0};
    const solver_int dimhArr[NSTAGES] = {9, 9, 9, 9, 9, 9, 9, 9, 9, 9};
    const solver_int dimpArr[NSTAGES] = {61, 61, 61, 61, 61, 61, 61, 61, 61, 61};
    const solver_int dimlArr[NSTAGES] = {5, 9, 9, 9, 9, 9, 9, 9, 9, 9};
    const solver_int dimuArr[NSTAGES] = {5, 9, 9, 9, 9, 9, 9, 9, 9, 9};
    const solver_int dimhlArr[NSTAGES] = {7, 7, 7, 7, 7, 7, 7, 7, 7, 7};
    const solver_int dimhuArr[NSTAGES] = {3, 3, 3, 3, 3, 3, 3, 3, 3, 3};
...
void mexFunction( solver_int nlhs, mxArray *plhs[], solver_int nrhs, const mxArray *prhs[] )  
{
    ...
    arr = prhs[0];
    copyMArrayToC_gp_mpcc_h2h_ego(mxGetPr(arr), z, nvar);
    if (nrhs > 1)
	{
        arr = prhs[1];
        if ( !mxIsEmpty(arr) )
        {
            copyMArrayToC_gp_mpcc_h2h_ego(mxGetPr(arr), p, dimp);
        }
	}   
	...
	copyCArrayToM_gp_mpcc_h2h_ego(obj, mxGetPr(obj_mex), 1);
	copyCArrayToM_gp_mpcc_h2h_ego(jacobj, mxGetPr(gradobj_mex), nvar);
	plhs[0] = obj_mex;
	plhs[1] = gradobj_mex;
