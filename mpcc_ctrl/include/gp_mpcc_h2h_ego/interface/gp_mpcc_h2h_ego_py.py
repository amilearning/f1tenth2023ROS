...
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
   ...
   OUTPUT = gp_mpcc_h2h_ego_py.gp_mpcc_h2h_ego_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['x0'] - matrix of size [190x1]
       PARAMS['xinit'] - matrix of size [14x1]
       PARAMS['all_parameters'] - matrix of size [610x1]

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['x01'] - column vector of length 19
       OUTPUT['x02'] - column vector of length 19
       OUTPUT['x03'] - column vector of length 19
       OUTPUT['x04'] - column vector of length 19
       OUTPUT['x05'] - column vector of length 19
       OUTPUT['x06'] - column vector of length 19
       OUTPUT['x07'] - column vector of length 19
       OUTPUT['x08'] - column vector of length 19
       OUTPUT['x09'] - column vector of length 19
       OUTPUT['x10'] - column vector of length 19

   [OUTPUT, EXITFLAG] = gp_mpcc_h2h_ego_py.gp_mpcc_h2h_ego_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = gp_mpcc_h2h_ego_py.gp_mpcc_h2h_ego_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'gp_mpcc_h2h_ego/lib/gp_mpcc_h2h_ego_withModel.so'))
    csolver = getattr(_lib,'gp_mpcc_h2h_ego_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'gp_mpcc_h2h_ego/lib/libgp_mpcc_h2h_ego_withModel.so'))
    csolver = getattr(_lib,'gp_mpcc_h2h_ego_solve')

internal_mem = getattr(_lib,'gp_mpcc_h2h_ego_internal_mem')

class gp_mpcc_h2h_ego_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('x0', ctypes.c_double * 190),
('xinit', ctypes.c_double * 14),
('all_parameters', ctypes.c_double * 610),
]

gp_mpcc_h2h_ego_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
gp_mpcc_h2h_ego_params_types = {'x0' : np.float64,
'xinit' : np.float64,
'all_parameters' : np.float64,
}

class gp_mpcc_h2h_ego_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('x01', ctypes.c_double * 19),
...
gp_mpcc_h2h_ego_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class gp_mpcc_h2h_ego_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class gp_mpcc_h2h_ego_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(gp_mpcc_h2h_ego_params_ctypes), ctypes.POINTER(gp_mpcc_h2h_ego_outputs_ctypes), ctypes.POINTER(gp_mpcc_h2h_ego_info), ctypes.POINTER(gp_mpcc_h2h_ego_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(gp_mpcc_h2h_ego_mem)

def gp_mpcc_h2h_ego_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = gp_mpcc_h2h_ego_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=gp_mpcc_h2h_ego_params_types[par], requirements='F')
    ...
    outputs_py = gp_mpcc_h2h_ego_outputs_ctypes()
    info_py = gp_mpcc_h2h_ego_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.gp_mpcc_h2h_ego_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.gp_mpcc_h2h_ego_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.gp_mpcc_h2h_ego_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.gp_mpcc_h2h_ego_adtool2forces)
            else:
                exitflag = _lib.gp_mpcc_h2h_ego_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.gp_mpcc_h2h_ego_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in gp_mpcc_h2h_ego_outputs:
        gp_mpcc_h2h_ego_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return gp_mpcc_h2h_ego_outputs,int(exitflag),info_py

solve = gp_mpcc_h2h_ego_solve
