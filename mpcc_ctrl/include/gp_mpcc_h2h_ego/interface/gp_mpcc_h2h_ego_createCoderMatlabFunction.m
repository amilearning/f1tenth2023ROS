...
% 
% gp_mpcc_h2h_ego_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the gp_mpcc_h2h_ego FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% gp_mpcc_h2h_ego_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% gp_mpcc_h2h_ego_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% gp_mpcc_h2h_ego_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function gp_mpcc_h2h_ego_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'gp_mpcc_h2h_ego_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'gp_mpcc_h2h_ego_block';
    end
    ...
    function_name = 'gp_mpcc_h2h_ego_coderFunction.m';
    position = [170, 99, 650, 350];
    parameter_sizes = struct('x0', '[190,1]',...
                             'xinit', '[14,1]',...
                             'all_parameters', '[610,1]');
    if useCompactBlock
        function_name = 'gp_mpcc_h2h_ego_coderFunctioncompact.m';
