...
public:
    ...
    : track_width(track_width), slack(slack) {
        ...
        Pose globalPose = {2.1, -0.39, 0}; // Example global pose
        auto [s, ey, epsi] = globalToLocal(globalPose);
        FrenPose f = {s, ey, epsi};
        auto [x, y, psi] = localToGlobal(f);
        std::cout << "Local coordinates: s = " << s << ", ey = " << ey << ", epsi = " << epsi*180/M_PI << std::endl;
        std::cout << "Global coordinates:x= " << x << ", y= " << y << ", psi = " << psi*180/M_PI << std::endl;
    ...
    void plot() {
        ...
        // Note: C++ does not have a direct equivalent to Python's matplotlib. 
        // You may need to use a library like OpenCV or Matplotlib for C++ for plotting.
    }
};
