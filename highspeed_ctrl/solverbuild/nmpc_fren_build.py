...
def continuous_dynamics(x, u):
    ...
    l_r = 0.15 # distance rear wheels to center of gravitiy of the car
    l_f = 0.15 # distance front wheels to center of gravitiy of the car
    m = 0.2   # mass of the car
...
def generate_pathplanner():
    ...
    model.lb = np.array([-5.,  np.deg2rad(-90.),  -200.,   -200.,  0.,   -np.inf, -0.48*np.pi])
    model.ub = np.array([+5.,  np.deg2rad(+90.),   200.,   200.,   20.,   np.inf,  0.48*np.pi])
